...
func get_wind_at_position(position: Vector3) -> Vector3:
	...
	var z_strenght_factor := (
		sqrt(0.5 * ridge_lift.width - abs(position_in_lift.z)) / squared_half_width
	)
...
func get_curve_baked_t_at_x(x: float) -> float:
	var curve_span := absf(
		curve.get_point_position(curve.point_count - 1).x - curve.get_point_position(0).x
	)
	var t := x
	var point := curve.sample_baked(t)
	var t_shift := curve_span * SAMPLING_RANGE_FACTOR
	while (
		t_shift > SAMPLING_T_STOP_THRESHOLD and absf(point.x - x) > SAMPLING_DISTANCE_STOP_THRESHOLD
	):
		if point.x > x:
			t -= t_shift
		else:
	...
	if absf(point.x - x) > SAMPLING_DISTANCE_STOP_THRESHOLD:
		push_warning(
			"Failed to interpolate good sampling for x=%.1f, t=%.1f, p_x=%.1f" % [x, t, point.x]
		)
	return t
